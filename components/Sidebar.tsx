...
export default function Sidebar() {
  ...
  return (
    <>
      <aside className="hidden md:flex flex-col w-64 h-screen sticky top-0" style={{backgroundColor: '#0f172a', borderRight: '1px solid #1e293b'}}>
        <div className="p-6" style={{borderBottom: '1px solid #1e293b'}}>
          <div className="flex items-center gap-3">
            ...
            <span className="font-display font-bold text-white text-lg">FleetPro</span>
          </div>
        </div>
        <nav className="flex-1 p-4 space-y-1">
          {nav.map(item => (
            <Link
              ...
              style={pathname === item.href ? {backgroundColor: 'rgba(59,91,219,0.1)', border: '1px solid rgba(59,91,219,0.2)'} : {}}
            >
              {item.icon}
              {item.label}
            </Link>
        ...
        <div className="p-4" style={{borderTop: '1px solid #1e293b'}}>
          <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-slate-500 hover:text-red-400 w-full transition-all duration-150" style={{hover: {backgroundColor: 'rgba(239,68,68,0.1)'}}}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 overflow-x-auto" style={{backgroundColor: '#0f172a', borderTop: '1px solid #1e293b'}}>
        <div className="flex min-w-max">
          {nav.map(item => (
            <Link
              key={item.href}
              href={item.href}
              className="flex flex-col items-center gap-1 py-3 px-4 text-xs transition-all duration-150"
              style={{color: pathname === item.href ? '#3b5bdb' : '#64748b'}}
            >
              {item.icon}
