...
export default function ReservationsPage() {
  ...
  async function load() {
    ...
    const { data: v } = await supabase.from('vehicles').select('id, brand, model, plate').eq('status', 'available')
    if (v) setVehicles(v)
  }
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface-900 border border-surface-800 rounded-2xl p-6 w-full max-w-md">
            <h2 className="font-display text-xl font-semibold text-white mb-6">Nueva reserva</h2>
